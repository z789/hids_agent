...
#define PATH_PID_FILENAME      "/var/opt/ucon_exec_null_pid"
...
static int out_event_exec(FILE * out, struct exec_event *ev)
{
	...
	snprintf(s_cred, sizeof(s_cred), "uid:%d euid:%d suid:%d "
		 "fsuid:%d uid:%d euid:%d suid:%d fsuid:%d",
	...
	ret = fprintf(out, "%s: EXEC: pid:%d tgid:%d ppid:%d ptgid:%d "
		      "cred:%s comm:%s exe:%s cmdline:%s\n",
		      ns_to_date(ev->timestamp_ns),
		      //ev->timestamp_ns,
		      ev->event_data.exec.process_pid,
		      ev->event_data.exec.process_tgid,
		      ev->event_data.exec.parent_pid,
		      ev->event_data.exec.parent_tgid, s_cred,
...
static int out_event_connect(FILE * out, struct exec_event *ev)
{
	...
	ret = fprintf(out, "%s: CONNECT: pid:%d tgid:%d comm:%s addr:%s:%d\n",
		      ns_to_date(ev->timestamp_ns),
		      pid, tgid, p_comm, inet_ntoa(addr->sin_addr),
		      ntohs(addr->sin_port));
...
static int out_event_mmap(FILE * out, struct exec_event *ev)
{
	...
	ret = fprintf(out, "%s: MMAP: pid:%d tgid:%d comm:%s path:%s\n",
		      ns_to_date(ev->timestamp_ns), pid, tgid, p_comm, path);
...
static int open_netlink_connect(void)
{
	...
	l_local.nl_family = AF_NETLINK;
	//l_local.nl_groups = CN_NETLINK_USERS + 0xf;   /* bitmask of requested groups */
	l_local.nl_groups = CN_IDX_EXEC;	/* bitmask of requested groups */
	l_local.nl_pid = getpid();
...
#if 0
	{
		int on = CN_IDX_EXEC;	/* Additional group number */
		setsockopt(s, SOL_NETLINK, NETLINK_DROP_MEMBERSHIP, &on,
			   sizeof(on));
	}
#endif
